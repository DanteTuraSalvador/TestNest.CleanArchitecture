...
public class EmployeeRepositoryTests
{
    ...
    private static void SeedData(ApplicationDbContext context)
    {
        ...
        Employee employee1 = Employee.Create(EmployeeNumber.Create("EMP-001").Value!, PersonName.Create("John", null, "Doe").Value!, EmailAddress.Create("john.doe@example.com").Value!, new EmployeeRoleId(Guid.NewGuid()), establishment1.Id).Value!;
        Employee employee2 = Employee.Create(EmployeeNumber.Create("EMP-002").Value!, PersonName.Create("Jane", null, "Smith").Value!, EmailAddress.Create("jane.smith@example.com").Value!, new EmployeeRoleId(Guid.NewGuid()), establishment2.Id).Value!;
    ...
    public async Task GetByIdAsync_ExistingEmployee_ReturnsSuccessResultWithEmployee()
    {
        ...
        EmployeeId existingEmployeeId = context.Employees.First().Id;
    ...
    public async Task UpdateAsync_ExistingEmployee_ReturnsSuccessResultWithUpdatedEmployee()
    {
        ...
        using ApplicationDbContext context = CreateContext();
        var repository = new EmployeeRepository(context);
        Employee existingEmployee = await context.Employees.FirstAsync();
        ...
        Assert.True(result.IsSuccess);
        Assert.NotNull(result.Value);
        Employee? retrievedEmployee = await context.Employees.FindAsync(existingEmployee.Id);
    ...
    public async Task DeleteAsync_ExistingEmployee_ReturnsSuccessResultAndDeletesEmployee()
    {
        ...
        // Assert
        Assert.True(result.IsSuccess);
        var deletedEmployee = await context.Employees.FindAsync(existingEmployeeId);
