...
/// <summary>
/// Manages establishment addresses including creation, retrieval, partial updates, and deletion with support for pagination and filtering.
/// </summary>
[ApiController]
[ApiVersion("1.0")]
    ...
    IErrorResponseService errorResponseService) : ControllerBase
{
    ...
    /// <response code="403">If the user does not have Manager or Admin role.</response>
    /// <response code="500">If an internal server error occurs during the creation process.</response>
    [HttpPost]
    [Authorize(Policy = Policies.RequireManagerOrAdmin)]
    ...
    /// <summary>
    /// Partially updates an establishment address using JSON Patch operations.
    /// </summary>
    /// <param name="establishmentAddressId">The unique identifier of the establishment address to update.</param>
    /// <param name="patchDocument">The JSON Patch document containing the update operations.</param>
    /// <returns>The updated establishment address.</returns>
    /// <response code="200">Returns the updated establishment address.</response>
    /// <response code="400">If the patch document is invalid or validation fails.</response>
    /// <response code="403">If the user does not have Manager or Admin role.</response>
    /// <response code="404">If the establishment address is not found.</response>
