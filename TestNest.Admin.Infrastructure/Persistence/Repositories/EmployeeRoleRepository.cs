    ...
    : GenericRepository<EmployeeRole, EmployeeRoleId>(employeeRoleDbContext), IEmployeeRoleRepository
{
    ...
    public override async Task<Result> DeleteAsync(EmployeeRoleId employeeRoleId)
    {
        ...
        {
            EmployeeRole? employeeRole = await _employeeRoleDbContext.EmployeeRoles
                .FirstOrDefaultAsync(p => p.Id == employeeRoleId);

            if (employeeRole == null)
            {
                return Result.Failure(ErrorType.NotFound,
                    new Error(EmployeeRoleException.NotFound().Code.ToString(),
                        EmployeeRoleException.NotFound().Message.ToString()));
            }

            // Remove will trigger soft delete interceptor
            _ = _employeeRoleDbContext.EmployeeRoles.Remove(employeeRole);
